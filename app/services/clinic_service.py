...
class ClinicService:
        ...
        try:
            ...
            if existing_clinic:
                ...
                clinic_doc = {
                    ...
                    "last_reset_date": now,
                    # ElevenLabs лимиты в минутах
                    "monthly_limit_minutes": 3000,  # 3000 минут = 50 часов
                    "weekly_limit_minutes": 750,     # 750 минут = 12.5 часов (1/4 месячного)
                    "current_month_minutes": 0,
                    # Метод определения администратора
                    "admin_detection_method": "amocrm",  # "amocrm" или "ai_schedule"
                    "current_week_minutes": 0,
                    "last_week_reset_date": now,
                    "created_at": now,
                    "updated_at": now,
    ...
    async def get_datalens_url_by_client_id(self, client_id: str) -> Optional[str]:
        """
        Находит клинику по client_id и возвращает URL дашборда DataLens.
        """
        try:
            clinic = await self.db.clinics.find_one(
                {"client_id": client_id},
                {"datalens_dashboard_url": 1}  # Проекция для получения только нужного поля
            )
            if clinic:
                return clinic.get("datalens_dashboard_url")
            return None
        except Exception as e:
            logger.error(f"Ошибка при поиске URL DataLens для client_id {client_id}: {e}")
            return None
    ...
    async def get_all_clinics(self) -> List[Dict[str, Any]]:
        """
        Возвращает список всех клиник с их администраторами.
        """
        try:
            clinics_cursor = self.db.clinics.find()
            all_clinics = []
            async for clinic in clinics_cursor:
                clinic_id = str(clinic["_id"])
                admin_cursor = self.db.administrators.find({"clinic_id": clinic_id})
                administrators = []
                async for admin in admin_cursor:
                    administrators.append(
                        {
                ...
                clinic_data = {
                    ...
                    "datalens_dashboard_url": clinic.get("datalens_dashboard_url"),
                    "administrators": administrators,
                }
                all_clinics.append(clinic_data)
            return all_clinics
        except Exception as e:
            logger.error(f"Ошибка при получении списка всех клиник: {e}")
            return []

    async def update_datalens_url(self, client_id: str, url: str) -> Optional[Dict[str, Any]]:
        """
        Обновляет URL дашборда DataLens для клиники по ее client_id.
        """
        try:
            now = datetime.now().isoformat()
            result = await self.db.clinics.find_one_and_update(
                {"client_id": client_id},
                {"$set": {"datalens_dashboard_url": url, "updated_at": now}},
    ...
    async def update_clinic_settings(self, clinic_id: str, settings: dict) -> bool:
        """
        Обновление настроек клиники
        
        Args:
            clinic_id: ID клиники (_id или client_id)
        ...
        """
        try:
            allowed_settings = ["admin_detection_method"]
            filtered_settings = {k: v for k, v in settings.items() if k in allowed_settings}
            
            if not filtered_settings:
                logger.warning(f"Нет валидных настроек для обновления клиники {clinic_id}")
                return False
            
            # Валидация admin_detection_method
            if "admin_detection_method" in filtered_settings:
                if filtered_settings["admin_detection_method"] not in ["amocrm", "ai_schedule"]:
                    logger.error(f"Неверное значение admin_detection_method: {filtered_settings['admin_detection_method']}")
                    return False
            
            # Сначала находим клинику, используя ту же логику, что и в get_clinic_by_id
            clinic = None
            ...
            # 4. Пробуем по полю client_id
            if not clinic:
                clinic = await self.db.clinics.find_one({"client_id": clinic_id})
            
            if not clinic:
                logger.warning(f"Клиника {clinic_id} не найдена")
                return False
            
            # Обновляем настройки используя реальный _id из найденной клиники
            result = await self.db.clinics.update_one(
                {"_id": clinic["_id"]},
                {"$set": filtered_settings}
            )
            
            if result.modified_count > 0:
                logger.info(f"Настройки клиники {clinic_id} обновлены: {filtered_settings}")
                return True
            else:
                logger.warning(f"Настройки клиники {clinic_id} не изменились (возможно, уже установлены)")
                return False
